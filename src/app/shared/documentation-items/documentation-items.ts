import { Injectable } from '@angular/core';

export interface DocItem {
  id: string;
  name: string;
  packageName?: string;
  examples?: string[];
}

export interface DocCategory {
  id: string;
  name: string;
  items: DocItem[];
  summary?: string;
}
const COMPONENTS = 'components';

const DOCS: {[key: string]: DocCategory[]} = {
  [COMPONENTS]: [
    {
      id: 'buttons&indicators',
      name: 'Buttons & Indicators',
      items: [
        {id: 'tablevizdifferelement', name: 'TableVizDifferElement', examples: ['table-viz-differ-element']},
        {id: 'tablevizmultiheader', name: 'TableVizMultiHeader', examples: ['table-viz-multi-header']},
        {id: 'tablevizbasic', name: 'TableVizBasic', examples: ['table-viz-basic']},
        {id: 'tablestickyheader', name: 'TableStickyHeader', examples: ['table-sticky-header']},
        {id: 'tablestickyfooter', name: 'TableStickyFooter', examples: ['table-sticky-footer']},
        {id: 'tablestickycolumns', name: 'TableStickyColumns', examples: ['table-sticky-columns']},
        {id: 'tablesorting', name: 'TableSorting', examples: ['table-sorting']},
        {id: 'tableselection', name: 'TableSelection', examples: ['table-selection']},
        {id: 'tablerowcontext', name: 'TableRowContext', examples: ['table-row-context']},
        {id: 'tablepagination', name: 'TablePagination', examples: ['table-pagination']},
        {id: 'tableoverview', name: 'TableOverview', examples: ['table-overview']},
        {id: 'tablemultipleheaderfooter', name: 'TableMultipleHeaderFooter', examples: ['table-multiple-header-footer']},
        {id: 'tablehttp', name: 'TableHttp', examples: ['table-http']},
        {id: 'tablefooterrow', name: 'TableFooterRow', examples: ['table-footer-row']},
        {id: 'tablefiltering', name: 'TableFiltering', examples: ['table-filtering']},
        {id: 'tableexpandablerows', name: 'TableExpandableRows', examples: ['table-expandable-rows']},
        {id: 'tabledynamiccolumns', name: 'TableDynamicColumns', examples: ['table-dynamic-columns']},
        {id: 'tablebasicflex', name: 'TableBasicFlex', examples: ['table-basic-flex']},
        {id: 'tablebasic', name: 'TableBasic', examples: ['table-basic']},
        {id: 'table', name: 'Table', examples: ['table']},
        {id: 'toolbaroverview', name: 'ToolbarOverview', examples: ['toolbar-overview']},
        {id: 'toolbarmultirow', name: 'ToolbarMultirow', examples: ['toolbar-multirow']},
        {id: 'toolbar', name: 'Toolbar', examples: ['toolbar']},
        {id: 'toolbar', name: 'Toolbar', examples: ['toolbar']},
        {id: 'sidenavbackdrop', name: 'SidenavBackdrop', examples: ['sidenav-backdrop']},
        {id: 'sidenavresponsive', name: 'SidenavResponsive', examples: ['sidenav-responsive']},
        {id: 'sidenavfixed', name: 'SidenavFixed', examples: ['sidenav-fixed']},
        {id: 'sidenavautosize', name: 'SidenavAutosize', examples: ['sidenav-autosize']},
        {id: 'sidenavdisableclose', name: 'SidenavDisableClose', examples: ['sidenav-disable-close']},
        {id: 'sidenavmode', name: 'SidenavMode', examples: ['sidenav-mode']},
        {id: 'sidenavopenclose', name: 'SidenavOpenClose', examples: ['sidenav-open-close']},
        {id: 'sidenavposition', name: 'SidenavPosition', examples: ['sidenav-position']},
        {id: 'sidenavdraweroverview', name: 'SidenavDrawerOverview', examples: ['sidenav-drawer-overview']},
        {id: 'sidenavoverview', name: 'SidenavOverview', examples: ['sidenav-overview']},
        {id: 'sidenav', name: 'Sidenav', examples: ['sidenav']},
        {id: 'sidenav', name: 'Sidenav', examples: ['sidenav']},
        {id: 'progressbarquery', name: 'ProgressBarQuery', examples: ['progress-bar-query']},
        {id: 'progressbarbuffer', name: 'ProgressBarBuffer', examples: ['progress-bar-buffer']},
        {id: 'progressbarindeterminate', name: 'ProgressBarIndeterminate', examples: ['progress-bar-indeterminate']},
        {id: 'progressbardeterminate', name: 'ProgressBarDeterminate', examples: ['progress-bar-determinate']},
        {id: 'progressbarconfigurable', name: 'ProgressBarConfigurable', examples: ['progress-bar-configurable']},
        {id: 'progressbar', name: 'ProgressBar', examples: ['progress-bar']},
        {id: 'selecterrorstatematcher', name: 'SelectErrorStateMatcher', examples: ['select-error-state-matcher']},
        {id: 'selectpanelclass', name: 'SelectPanelClass', examples: ['select-panel-class']},
        {id: 'selectnoripple', name: 'SelectNoRipple', examples: ['select-no-ripple']},
        {id: 'selectcustomtrigger', name: 'SelectCustomTrigger', examples: ['select-custom-trigger']},
        {id: 'selectmultiple', name: 'SelectMultiple', examples: ['select-multiple']},
        {id: 'selectoptgroup', name: 'SelectOptgroup', examples: ['select-optgroup']},
        {id: 'selectreset', name: 'SelectReset', examples: ['select-reset']},
        {id: 'selectdisabled', name: 'SelectDisabled', examples: ['select-disabled']},
        {id: 'selecthinterror', name: 'SelectHintError', examples: ['select-hint-error']},
        {id: 'selectform', name: 'SelectForm', examples: ['select-form']},
        {id: 'selectvaluebinding', name: 'SelectValueBinding', examples: ['select-value-binding']},
        {id: 'selectoverview', name: 'SelectOverview', examples: ['select-overview']},
        {id: 'select', name: 'Select', examples: ['select']},
        {id: 'select', name: 'Select', examples: ['select']},
        {id: 'inputprefixsuffix', name: 'InputPrefixSuffix', examples: ['input-prefix-suffix']},
        {id: 'inputhint', name: 'InputHint', examples: ['input-hint']},
        {id: 'inputform', name: 'InputForm', examples: ['input-form']},
        {id: 'inputerrors', name: 'InputErrors', examples: ['input-errors']},
        {id: 'inputclearable', name: 'InputClearable', examples: ['input-clearable']},
        {id: 'textfieldautosizetextarea', name: 'TextFieldAutosizeTextarea', examples: ['text-field-autosize-textarea']},
        {id: 'inputerrorstatematcher', name: 'InputErrorStateMatcher', examples: ['input-error-state-matcher']},
        {id: 'inputoverview', name: 'InputOverview', examples: ['input-overview']},
        {id: 'input', name: 'Input', examples: ['input']},
        {id: 'input', name: 'Input', examples: ['input']},
        {id: 'formfieldcustomcontrol', name: 'FormFieldCustomControl', examples: ['form-field-custom-control']},
        {id: 'formfieldtheming', name: 'FormFieldTheming', examples: ['form-field-theming']},
        {id: 'formfieldprefixsuffix', name: 'FormFieldPrefixSuffix', examples: ['form-field-prefix-suffix']},
        {id: 'formfielderror', name: 'FormFieldError', examples: ['form-field-error']},
        {id: 'formfieldhint', name: 'FormFieldHint', examples: ['form-field-hint']},
        {id: 'formfieldappearance', name: 'FormFieldAppearance', examples: ['form-field-appearance']},
        {id: 'formfieldlabel', name: 'FormFieldLabel', examples: ['form-field-label']},
        {id: 'formfieldoverview', name: 'FormFieldOverview', examples: ['form-field-overview']},
        {id: 'formfield', name: 'FormField', examples: ['form-field']},
        {id: 'formfield', name: 'FormField', examples: ['form-field']},
        {id: 'expansionsteps', name: 'ExpansionSteps', examples: ['expansion-steps']},
        {id: 'expansionoverview', name: 'ExpansionOverview', examples: ['expansion-overview']},
        {id: 'expansion', name: 'Expansion', examples: ['expansion']},
        {id: 'gridlistoverview', name: 'GridListOverview', examples: ['grid-list-overview']},
        {id: 'gridlistdynamic', name: 'GridListDynamic', examples: ['grid-list-dynamic']},
        {id: 'gridlist', name: 'GridList', examples: ['grid-list']},
        {id: 'listselection', name: 'ListSelection', examples: ['list-selection']},
        {id: 'listoverview', name: 'ListOverview', examples: ['list-overview']},
        {id: 'listsections', name: 'ListSections', examples: ['list-sections']},
        {id: 'list', name: 'List', examples: ['list']},
        {id: 'list', name: 'List', examples: ['list']},
        {id: 'tooltipautohide', name: 'TooltipAutoHide', examples: ['tooltip-auto-hide']},
        {id: 'tooltipmodifieddefaults', name: 'TooltipModifiedDefaults', examples: ['tooltip-modified-defaults']},
        {id: 'tooltipmessage', name: 'TooltipMessage', examples: ['tooltip-message']},
        {id: 'tooltipmanual', name: 'TooltipManual', examples: ['tooltip-manual']},
        {id: 'tooltipdisabled', name: 'TooltipDisabled', examples: ['tooltip-disabled']},
        {id: 'tooltipdelay', name: 'TooltipDelay', examples: ['tooltip-delay']},
        {id: 'tooltipcustomclass', name: 'TooltipCustomClass', examples: ['tooltip-custom-class']},
        {id: 'tooltipposition', name: 'TooltipPosition', examples: ['tooltip-position']},
        {id: 'tooltipoverview', name: 'TooltipOverview', examples: ['tooltip-overview']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'objectobject', name: 'ObjectObject', examples: ['[object Object]']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'toolkit', name: 'Toolkit', examples: ['toolkit']},
        {id: 'toolkit', name: 'Toolkit', examples: ['toolkit']},
        {id: 'toolkit', name: 'Toolkit', examples: ['toolkit']},
        {id: 'toolkit', name: 'Toolkit', examples: ['toolkit']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltipautohide', name: 'TooltipAutoHide', examples: ['tooltip-auto-hide']},
        {id: 'tooltipmodifieddefaults', name: 'TooltipModifiedDefaults', examples: ['tooltip-modified-defaults']},
        {id: 'tooltipmessage', name: 'TooltipMessage', examples: ['tooltip-message']},
        {id: 'tooltipmanual', name: 'TooltipManual', examples: ['tooltip-manual']},
        {id: 'tooltipdisabled', name: 'TooltipDisabled', examples: ['tooltip-disabled']},
        {id: 'tooltipdelay', name: 'TooltipDelay', examples: ['tooltip-delay']},
        {id: 'tooltipcustomclass', name: 'TooltipCustomClass', examples: ['tooltip-custom-class']},
        {id: 'tooltipposition', name: 'TooltipPosition', examples: ['tooltip-position']},
        {id: 'tooltipoverview', name: 'TooltipOverview', examples: ['tooltip-overview']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'tooltip', name: 'Tooltip', examples: ['tooltip']},
        {id: 'divideroverview', name: 'DividerOverview', examples: ['divider-overview']},
        {id: 'datepickerdateclass', name: 'DatepickerDateClass', examples: ['datepicker-date-class']},
        {id: 'datepickercustomheader', name: 'DatepickerCustomHeader', examples: ['datepicker-custom-header']},
        {id: 'datepickercolor', name: 'DatepickerColor', examples: ['datepicker-color']},
        {id: 'datepickerviewsselection', name: 'DatepickerViewsSelection', examples: ['datepicker-views-selection']},
        {id: 'datepickercustomicon', name: 'DatepickerCustomIcon', examples: ['datepicker-custom-icon']},
        {id: 'datepickerformats', name: 'DatepickerFormats', examples: ['datepicker-formats']},
        {id: 'datepickermoment', name: 'DatepickerMoment', examples: ['datepicker-moment']},
        {id: 'datepickerlocale', name: 'DatepickerLocale', examples: ['datepicker-locale']},
        {id: 'datepickerapi', name: 'DatepickerApi', examples: ['datepicker-api']},
        {id: 'datepickertouch', name: 'DatepickerTouch', examples: ['datepicker-touch']},
        {id: 'datepickerdisabled', name: 'DatepickerDisabled', examples: ['datepicker-disabled']},
        {id: 'datepickerevents', name: 'DatepickerEvents', examples: ['datepicker-events']},
        {id: 'datepickerfilter', name: 'DatepickerFilter', examples: ['datepicker-filter']},
        {id: 'datepickerminmax', name: 'DatepickerMinMax', examples: ['datepicker-min-max']},
        {id: 'datepickervalue', name: 'DatepickerValue', examples: ['datepicker-value']},
        {id: 'datepickerstartview', name: 'DatepickerStartView', examples: ['datepicker-start-view']},
        {id: 'datepickeroverview', name: 'DatepickerOverview', examples: ['datepicker-overview']},
        {id: 'treeloadmore', name: 'TreeLoadmore', examples: ['tree-loadmore']},
        {id: 'treenestedoverview', name: 'TreeNestedOverview', examples: ['tree-nested-overview']},
        {id: 'treechecklist', name: 'TreeChecklist', examples: ['tree-checklist']},
        {id: 'treeflatoverview', name: 'TreeFlatOverview', examples: ['tree-flat-overview']},
        {id: 'treedynamic', name: 'TreeDynamic', examples: ['tree-dynamic']},
        {id: 'tree', name: 'Tree', examples: ['tree']},
        {id: 'tree', name: 'Tree', examples: ['tree']},
        {id: 'dialogcontent', name: 'DialogContent', examples: ['dialog-content']},
        {id: 'dialogdata', name: 'DialogData', examples: ['dialog-data']},
        {id: 'dialogoverview', name: 'DialogOverview', examples: ['dialog-overview']},
        {id: 'dialog', name: 'Dialog', examples: ['dialog']},
        {id: 'dialog', name: 'Dialog', examples: ['dialog']},
        {id: 'dialogcontent', name: 'DialogContent', examples: ['dialog-content']},
        {id: 'dialogdata', name: 'DialogData', examples: ['dialog-data']},
        {id: 'dialogoverview', name: 'DialogOverview', examples: ['dialog-overview']},
        {id: 'snackbar', name: 'SnackBarOverview', examples: ['snack-bar-overview']},
        {id: 'snackbarcomponent', name: 'SnackBarComponent', examples: ['snack-bar-component']},
        {id: 'tabgroupanimations', name: 'TabGroupAnimations', examples: ['tab-group-animations']},
        {id: 'tabgroupalign', name: 'TabGroupAlign', examples: ['tab-group-align']},
        {id: 'tabnavbarbasic', name: 'TabNavBarBasic', examples: ['tab-nav-bar-basic']},
        {id: 'tabgroupasync', name: 'TabGroupAsync', examples: ['tab-group-async']},
        {id: 'tabgrouptheme', name: 'TabGroupTheme', examples: ['tab-group-theme']},
        {id: 'tabgroupstretched', name: 'TabGroupStretched', examples: ['tab-group-stretched']},
        {id: 'tabgrouplazyloaded', name: 'TabGroupLazyLoaded', examples: ['tab-group-lazy-loaded']},
        {id: 'tabgroupheaderbelow', name: 'TabGroupHeaderBelow', examples: ['tab-group-header-below']},
        {id: 'tabgroupdynamic', name: 'TabGroupDynamic', examples: ['tab-group-dynamic']},
        {id: 'tabgroupdynamicheight', name: 'TabGroupDynamicHeight', examples: ['tab-group-dynamic-height']},
        {id: 'tabgroupcustomlabel', name: 'TabGroupCustomLabel', examples: ['tab-group-custom-label']},
        {id: 'tabgroupbasic', name: 'TabGroupBasic', examples: ['tab-group-basic']},
        {id: 'tabs', name: 'Tabs', examples: ['tabs']},
        {id: 'buttonfloatingmini', name: 'ButtonFloatingMini', examples: ['button-floating-mini']},
        {id: 'buttonfloating', name: 'ButtonFloating', examples: ['button-floating']},
        {id: 'buttonicon', name: 'ButtonIcon', examples: ['button-icon']},
        {id: 'buttonflat', name: 'ButtonFlat', examples: ['button-flat']},
        {id: 'buttonraised', name: 'ButtonRaised', examples: ['button-raised']},
        {id: 'chipstacked', name: 'ChipStacked', examples: ['chip-stacked']},
        {id: 'chipwithinput', name: 'ChipWithInput', examples: ['chip-with-input']},
        {id: 'chipautocomplete', name: 'ChipAutocomplete', examples: ['chip-autocomplete']},
        {id: 'checkboxconfigurable', name: 'CheckboxConfigurable', examples: ['checkbox-configurable']},
        {id: 'slidetoggleconfigurable', name: 'SlideToggleConfigurable', examples: ['slide-toggle-configurable']},
        {id: 'radiowidthngmodel', name: 'RadioWidthNgmodel', examples: ['radio-width-ngmodel']},
        {id: 'sliderconfigurable', name: 'SliderConfigurable', examples: ['slider-configurable']},
        {id: 'sliderformatting', name: 'SliderFormatting', examples: ['slider-formatting']},
        {id: 'paginatorconfigurable', name: 'PaginatorConfigurable', examples: ['paginator-configurable']},
        {id: 'sorttable', name: 'SortTable', examples: ['sort-table']},
        {id: 'sort', name: 'SortOverview', examples: ['sort-overview']},
        {id: 'autocompleteoverview', name: 'AutocompleteOverview', examples: ['autocomplete-overview']},
        {id: 'autocompleteoptgroup', name: 'AutocompleteOptgroup', examples: ['autocomplete-optgroup']},
        {id: 'autocompletehighlight', name: 'AutocompleteHighlight', examples: ['autocomplete-highlight']},
        {id: 'autocompletedisplay', name: 'AutocompleteDisplay', examples: ['autocomplete-display']},
        {id: 'autocompletefilter', name: 'AutocompleteFilter', examples: ['autocomplete-filter']},
        {id: 'autocomplete', name: 'AutocompleteSimple', examples: ['autocomplete-simple']},
        {id: 'cardfancy', name: 'CardFancy', examples: ['card-fancy']},
        {id: 'ripples', name: 'RippleOverview', examples: ['ripple-overview']},
        {id: 'badgebutton', name: 'BadgeButton', examples: ['badge-button']},
        {id: 'badgetext', name: 'BadgeText', examples: ['badge-text']},
        {id: 'steppererrors', name: 'StepperErrors', examples: ['stepper-errors']},
        {id: 'stepperstates', name: 'StepperStates', examples: ['stepper-states']},
        {id: 'steppereditable', name: 'StepperEditable', examples: ['stepper-editable']},
        {id: 'stepperoptional', name: 'StepperOptional', examples: ['stepper-optional']},
        {id: 'stepperlabelpositionbottom', name: 'StepperLabelPositionBottom', examples: ['stepper-label-position-bottom']},
        {id: 'steppervertical', name: 'StepperVertical', examples: ['stepper-vertical']},
        {id: 'stepperoverview', name: 'StepperOverview', examples: ['stepper-overview']},
        {id: 'buttontogglesecondarycolor', name: 'ButtontoggleSecondaryColor', examples: ['buttontoggle-secondary-color']},
        {id: 'buttontoggletwoicon', name: 'ButtontoggleTwoIcon', examples: ['buttontoggle-two-icon']},
        {id: 'buttontogglethreeicon', name: 'ButtontoggleThreeIcon', examples: ['buttontoggle-three-icon']},
        {id: 'buttontoggletwotext', name: 'ButtontoggleTwoText', examples: ['buttontoggle-two-text']},
        {id: 'buttontoggledisable', name: 'ButtontoggleDisable', examples: ['buttontoggle-disable']},
        {id: 'buttontogglethreetext', name: 'ButtontoggleThreeText', examples: ['buttontoggle-three-text']},
        {id: 'buttontoggle', name: 'ButtontoggleBasic', examples: ['buttontoggle-basic']},
        {id: 'fileupload', name: 'FileUploadBasic', examples: ['file-upload-basic']},
        {id: 'selecttriggertext', name: 'SelectTriggerText', examples: ['select-trigger-text']},
        {id: 'selectmultipleselection', name: 'SelectMultipleSelection', examples: ['select-multiple-selection']},
        {id: 'selectgroupoption', name: 'SelectGroupOption', examples: ['select-group-option']},
        {id: 'selecterrorstate', name: 'SelectErrorState', examples: ['select-error-state']},
        {id: 'menuicon', name: 'MenuIcon', examples: ['menu-icon']},
        {id: 'datepickerrangevalidation', name: 'DatePickerRangeValidation', examples: ['date-picker-range-validation']},
        {id: 'datepickeremulating', name: 'DatePickerEmulating', examples: ['date-picker-emulating']},
        {id: 'datepickerfiltervalidation', name: 'DatePickerFilterValidation', examples: ['date-picker-filter-validation']},
        {id: 'datepickerminmaxvalidation', name: 'DatePickerMinMaxValidation', examples: ['date-picker-min-max-validation']},
        {id: 'datepickerselectedvalue', name: 'DatePickerSelectedValue', examples: ['date-picker-selected-value']},
        {id: 'datepickerstartdate', name: 'DatePickerStartDate', examples: ['date-picker-start-date']},
        {id: 'formfieldautocompletegroup', name: 'FormFieldAutocompleteGroup', examples: ['form-field-autocomplete-group']},
        {id: 'formfieldautocompleteicon', name: 'FormFieldAutocompleteIcon', examples: ['form-field-autocomplete-icon']},
        {id: 'formfieldcustomphonenumber', name: 'FormFieldCustomPhoneNumber', examples: ['form-field-custom-phone-number']},
        {id: 'formfieldprefixsuffix', name: 'FormFieldPrefixSuffix', examples: ['form-field-prefix-suffix']},
        {id: 'formfieldclearbutton', name: 'FormFieldClearButton', examples: ['form-field-clear-button']},
        {id: 'loginokta', name: 'LoginOkta', examples: ['login-okta']},
        {id: 'divider', name: 'Divider', examples: ['divider']},
        {id: 'iconography', name: 'Iconography', examples: ['iconography']},
        {id: 'steppercustom', name: 'StepperCustom', examples: ['stepper-custom']},
        {id: 'customstepper', name: 'CustomStepper', examples: ['custom-stepper']},
        {id: 'steppervertical', name: 'StepperVertical', examples: ['stepper-vertical']},
        {id: 'formfieldvalidation', name: 'FormFieldValidation', examples: ['form-field-validation']},
        {id: 'datepickersingleerror', name: 'DatePickerSingleError', examples: ['date-picker-single-error']},
        {id: 'customerselect', name: 'CustomerSelect', examples: ['customer-select']},
        {id: 'formfieldautocomplete', name: 'FormFieldAutocomplete', examples: ['form-field-autocomplete']},
        {id: 'monthpickerrange', name: 'MonthPickerRange', examples: ['month-picker-range']},
        {id: 'monthpickersingle', name: 'MonthPickerSingle', examples: ['month-picker-single']},
        {id: 'httpstatus', name: ''},
        {id: 'okta', name: ''},
        {id: 'applicationtemplates', name: ''},
        {id: 'colors', name: ''},
        {id: 'typography', name: ''},
        {id: 'search', name: 'SearchBasic', examples: ['search-basic']},
        {id: 'misc', name: 'Misc'},
        {id: 'breadscrumbbasic', name: 'BreadscrumbBasic', examples: ['breadscrumb-basic']},
        {id: 'bottomsheet', name: 'BottomsheetBasic', examples: ['bottomsheet-basic']},
        {id: 'stepper', name: 'StepperBasic', examples: ['stepper-basic']},
        {id: 'tablenative', name: 'TableNative', examples: ['table-native']},
        {id: 'table', name: 'TableBasic', examples: ['table-basic']},
        {id: 'progressspinner', name: 'ProgressSpinnerBasic', examples: ['progress-spinner-basic']},
        {id: 'sidenav', name: 'SidenavBasic', examples: ['sidenav-basic']},
        {id: 'feedbackindicatorwarning', name: 'FeedbackIndicatorWarning', examples: ['feedback-indicator-warning']},
        {id: 'feedbackindicator', name: 'FeedbackIndicatorBasic', examples: ['feedback-indicator-basic']},
        {id: 'nodata', name: 'NodataBasic', examples: ['nodata-basic']},
        {id: 'card', name: 'CardBasic', examples: ['card-basic']},
        {id: 'paginator', name: 'PaginatorBasic', examples: ['paginator-basic']},
        {id: 'tooltip', name: 'TooltipBasic', examples: ['tooltip-basic']},
        {id: 'footer', name: 'FooterBasic', examples: ['footer-basic']},
        {id: 'header', name: 'HeaderBasic', examples: ['header-basic']},
        {id: 'dialog', name: 'DialogBasic', examples: ['dialog-basic']},
        // {id: 'chipprimary', name: 'ChipPrimary', examples: ['chip-primary']},
        // {id: 'listnodivider', name: 'ListNodivider', examples: ['list-nodivider']},
        // {id: 'menunested', name: 'MenuNested', examples: ['menu-nested']},
        {id: 'button', name: 'Button', examples: ['button-basic']},
        {id: 'datepicker', name: 'DatePicker', examples: ['date-picker-basic']},
        {id: 'select', name: 'Select', examples: ['select-basic']},
        {id: 'checkbox', name: 'Checkbox', examples: ['checkbox-basic']},
        {id: 'menu', name: 'Menu', examples: ['menu-basic']},
        {id: 'formfield', name: 'Form field', examples: ['form-field-basic']},
        {id: 'radio', name: 'Radio button', examples: ['radio-button-basic']},
        {id: 'tree', name: 'Tree', examples: ['tree-basic']},
        {id: 'slider', name: 'Slider', examples: ['slider-basic']},
        {id: 'slidetoggle', name: 'Slide toggle', examples: ['slide-toggle-basic']},
        {id: 'icon', name: 'Icon', examples: ['icon-basic']},
        {id: 'toolbar', name: 'Toolbar', examples: ['toolbar-basic']},
        {id: 'list', name: 'List', examples: ['list-basic']},
        {id: 'chips', name: 'Chip', examples: ['chip-basic']},
        {id: 'badge', name: 'Badge', examples: ['badge-basic']},
        {id: 'progressbar', name: 'Progress Bar', examples: ['progress-bar-basic']},
        {id: 'headerfooter', name: 'Header Footer', examples: ['header-footer-basic']},
        {id: 'developers', name: 'Developers'},
        {id: 'designers', name: 'Designers'},
        {id: 'login', name: 'Login', examples: ['login-basic']},
      ]
    },
  ],
};

for (const category of DOCS[COMPONENTS]) {
  for (const doc of category.items) {
    doc.packageName = 'material';
  }
}

const ALL_COMPONENTS = DOCS[COMPONENTS].reduce(
  (result, category) => result.concat(category.items), []);
const ALL_DOCS = ALL_COMPONENTS;

@Injectable()
export class DocumentationItems {
  getCategories(section: string): DocCategory[] {
    return DOCS[section];
  }

  getItems(section: string): DocItem[] {
    return ALL_COMPONENTS;
  }

  getItemById(id: string): DocItem {
    return ALL_DOCS.find(doc => doc.id === id);
  }
}
